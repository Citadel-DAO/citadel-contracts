...
async function main() {
  ...
  if (
    fs.existsSync(path.join(scriptsDirectory, `${networkName}-mock-funding-addresses.json`))
  ) {
    fs.unlinkSync(path.join(scriptsDirectory, `${networkName}-mock-funding-addresses.json`));
  }
  fs.writeFileSync(
