...
contract VestingTest is BaseFixture {
    ...
    */
    function testClaimFlow() public {
        ...
        (, , uint256 lockedAmounts, uint256 claimedAmounts) = xCitadelVester
            .vesting(user);
        ...
        (uint256 unlockBegin, uint256 unlockEnds, , ) = xCitadelVester.vesting(
            user
        );
        assertEq(
            unlockEnds - unlockBegin,
            xCitadelVester.INITIAL_VESTING_DURATION()
        );
    ...
    */
    function testDoubleSimultaneousVestingFlow() public {
        ...
        assertEq(
            secondLockedAmounts,
            firstVestAmount - claimedAmounts + secondVestAmount
        ); // Total of two vestings minus the already claimed
        assertEq(secondClaimedAmounts, 0); // Claimed amounts are reset after a new vest
        assertEq(
            secondUnlockBegin,
            firstUnlockBegin + (xCitadelVester.vestingDuration() * 3) / 4
        ); // 3/4 of duration has elapsed since first lock
        assertEq(
            secondUnlockEnds,
            firstUnlockEnds + (xCitadelVester.vestingDuration() * 3) / 4
        ); // 3/4 of duration has elapsed since first lock
        ...
        assertEq(
            ...
            firstVestAmount + secondVestAmount - claimedAmounts
        );
    ...
    */
    function testDoubleConsecutiveVestingFlow() public {
        ...
        assertEq(
            lockedAmounts,
            firstVestAmount + secondVestAmount - firstVestAmount
        ); // First vest amount already claimed
